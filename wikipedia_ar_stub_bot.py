...
def process_page(page):
    try:
        ...
        # إذا كانت النتيجة أقل من 100، يتم إضافة قالب بذرة إذا لم يكن موجودًا
        if score < 100 and not re.search(r'{{بذرة\b', original_text):
        ...
        # إذا كانت النتيجة أكبر من أو تساوي 101، يتم حذف قالب بذرة إذا كان موجودًا
        elif score >= 101 and re.search(r'{{بذرة\b', original_text):
            # إزالة قالب "بذرة" من النص
            new_text = re.sub(r'{{بذرة\b.*?}}', '', original_text, flags=re.DOTALL).strip()
            
            # تحديث نص الصفحة
            page.text = new_text
            page.save(summary='إزالة قالب بذرة - تجريبي')
